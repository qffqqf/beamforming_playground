...
pres = - exp(1i*kWave*dist)./(4*pi*dist) - 0.0*exp(1i*kWave*dist_noise1)./(4*pi*dist_noise1)...
       - 0.1*exp(1i*kWave*dist_noise2)./(4*pi*dist_noise2);
Sp = pres*pres';
[V,eigen] = eig(Sp);
index_null = find(diag(eigen)<1e-5);
Un = V(:,index_null);
...
eta = 1e-15;
for iPix = 1:nPix
    energy(iPix) = 1/abs(G(:,iPix)'*Un*Un'*G(:,iPix) + eta);
end
